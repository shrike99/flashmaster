...
/* ─────────────────────────────────────────────────────────────────────────────
   SLEEK BLACK & WHITE FLASHCARD APP
        ...
        <div className="study-container">
          ...
          <div className="study-header">
            <div className="study-info">
              <h1>Study Mode</h1>
              <p>{si + 1} of {studyDeck.length}</p>
            </div>
            <button className="btn btn-outline btn-sm" onClick={() => setStudy("idle")}>Exit</button>
          </div>
          ...
          <div className="card">
            {showImgFront && (
              <img 
                ...
                alt="" 
                onClick={e => { e.stopPropagation(); setZoomImg(card.imgs[0]); }} 
              />
            )}
            <div className="card-content" style={{ minHeight: "200px", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ fontSize: "18px", lineHeight: "2", textAlign: "center" }}>
                {parts.map((p, i) => p.type === "text" ? <span key={i}>{p.val}</span> : (
                  <span key={i}>
                    {clozeChecked ? (
                      <>
                        <span style={{ 
                        ...
                        {(clozeAnswers[p.val] || "").trim().toLowerCase() !== p.val.toLowerCase() && (
                          <span style={{ color: "hsl(120 100% 35%)", fontWeight: 500, marginLeft: "8px" }}>({p.val})</span>
                        )}
                      </>
                    ) : (
                      <input
          ...
          {!clozeChecked ? (
            <div className="study-actions">
              <button className="btn btn-default" onClick={checkCloze}>Check Answers</button>
            </div>
          ) : (
            <div className="study-actions">
          ...
          <div className="keyboard-hint">
            ...
            <span className="kbd">←</span> or <span className="kbd">1</span> wrong · 
            <span className="kbd">→</span> or <span className="kbd">2</span> right
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="study-container">
        ...
        >
          <div
            ref={cardRef}
            className={`study-card${flipped ? " flipped" : ""}${flipped ? " swipeable" : ""}`}
            onClick={() => !isDragging && setFlipped(f => !f)}
          >
            {flipped && (
              <>
                <div className="swipe-indicator swipe-left" style={{ opacity: dragOffset.x < 0 ? swipeOpacity : 0 }}>✕</div>
                <div className="swipe-indicator swipe-right" style={{ opacity: dragOffset.x > 0 ? swipeOpacity : 0 }}>✓</div>
              </>
            )}
            
            <div className="card-face">
              {showImgFront && <img src={card.imgs[0]} className="card-image" alt="" onClick={e => { e.stopPropagation(); setZoomImg(card.imgs[0]); }} />}
              <div className="card-body">
                <div className="card-text markdown"><ReactMarkdown>{card.q}</ReactMarkdown></div>
                <div className="card-hint">Click to reveal answer</div>
              </div>
            </div>
            
            <div className="card-face card-face-back">
              {showImgBack && <img src={card.imgs[0]} className="card-image" alt="" onClick={e => { e.stopPropagation(); setZoomImg(card.imgs[0]); }} />}
              <div className="card-body">
                <div className="card-label">Answer</div>
                <div className="card-text markdown"><ReactMarkdown>{card.a}</ReactMarkdown></div>
              </div>
            </div>
          </div>
        </div>
        ...
        {flipped ? (
          ...
          <div style={{ marginTop: "24px", textAlign: "center", fontSize: "13px", color: "hsl(var(--muted-foreground))" }}>
            Flip the card first, then mark your result
          </div>
        )}
    ...
    return (
      <div className="study-container">
        <div className="study-header">
          ...
          <button className="btn btn-outline btn-sm" onClick={() => setStudy("idle")}>Done</button>
        </div>

        <div className="stats-grid">
          <div className="stat-card">
            <div className="stat-value" style={{ color: "hsl(120 100% 35%)" }}>{rightCount}</div>
            <div className="stat-label">Correct</div>
          </div>
          <div className="stat-card">
            <div className="stat-value" style={{ color: "hsl(var(--destructive))" }}>{wrongCount}</div>
        ...
        <div className="page-actions">
          ...
          {wrongCount > 0 && (
            <button className="btn btn-default" onClick={() => startStudy(Object.keys(results).filter(k => results[k] === "wrong").map(Number))}>
  ...
  return (
    <div>
      ...
      <div className="page-header">
        ...
        <p className="page-description">{cards.length} cards total · {dueCards.length} due for review</p>
              ...
              reader.onload = (event) => {
                try {
                  ...
                  if (Array.isArray(importedCards)) {
                    // Merge imported cards with existing ones, updating IDs to avoid conflicts
                  ...
                  {isCloze ? (
                    <div className="flashcard-question">
                      {parseCloze(card.template).map((p, i) =>
                        p.type === "text" ? <span key={i}>{p.val}</span> : <span key={i} style={{ color: "hsl(var(--foreground))", fontWeight: 600 }}>...</span>
                      )}
                    </div>
                  ) : (
                    <>
                      <div className="flashcard-question markdown"><ReactMarkdown>{card.q}</ReactMarkdown></div>
                      <div className="flashcard-answer markdown"><ReactMarkdown>{card.a}</ReactMarkdown></div>
                    </>
                  )}
                </div>
                
                <div className="flashcard-actions">
                  <button className="btn btn-ghost btn-icon-sm" onClick={e => { e.stopPropagation(); setEditing({ ...card, imgs: [...(card.imgs || [])], imgSide: card.imgSide || "both" }); }}>
        ...
        <div className="modal-overlay" onClick={() => setEditing(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              {editing.imgs?.length > 0 && (
                <>
                  <div style={{ position: "relative", display: "inline-block" }}>
                    ...
                    <button className="thumbnail-delete" onClick={() => setEditing(ed => ({ ...ed, imgs: [] }))}>×</button>
                  </div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-default" onClick={saveEdit}>Save</button>
            </div>
          </div>
        </div>
      )}
...
function AddCardsPage() {
  ...
  const [creatorCards, setCreatorCards] = useState([
    { id: 1, type: "basic", q: "", a: "", imgs: [], imgSide: "both" }
  ]);
  ...
  const addCreatorCard = () => {
    setCreatorCards(c => [...c, { id: creatorId.current++, type: "basic", q: "", a: "", imgs: [], imgSide: "both" }]);
  };
  ...
  const saveAllCards = () => {
    const valid = creatorCards.filter(c => {
      if (c.type === "cloze") {
        return c.template?.trim();
      }
      // For basic cards: must have a question, and either an answer OR an image
      return c.q.trim() && (c.a.trim() || (c.imgs && c.imgs.length > 0));
    });
    
    if (valid.length === 0) return;
    
    const newCards = valid.map(c => {
      // Make sure to preserve images when saving
      return {
        id: nid.current++,
        type: c.type,
        q: c.q || "",
  ...
  return (
    <div>
      ...
      <div className="page-header">
        ...
        <p className="page-description">Create multiple cards at once • Markdown supported • Paste images from clipboard</p>
      </div>
          ...
          >
            ...
            {card.type === "basic" ? (
              ...
              <div className="creator-section">
                ...
                <div style={{ fontSize: "12px", color: "hsl(var(--muted-foreground))", marginTop: "8px" }}>
                  Wrap blanks in {"{curly braces}"}
                </div>
              </div>
            )}
            ...
            <div className="creator-section">
              <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
                ...
                <input 
                  ...
                  accept="image/*" 
                  style={{ display: "none" }} 
                  onChange={e => readImgs(e.target.files, srcs => updateCreatorCard(card.id, "imgs", srcs))} 
                />
              ...
              {card.imgs?.length > 0 && (
                <div style={{ position: "relative", display: "inline-block", marginTop: "12px" }}>
                  ...
                  <button className="thumbnail-delete" onClick={() => updateCreatorCard(card.id, "imgs", [])}>×</button>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          <div className="stat-value" style={{ color: "hsl(120 100% 35%)" }}>{totalRight}</div>
          <div className="stat-label">Correct</div>
        </div>
        <div className="stat-card">
          <div className="stat-value" style={{ color: "hsl(var(--destructive))" }}>{totalWrong}</div>
          <div className="stat-label">Incorrect</div>
        </div>
        <div className="stat-card">
              ...
              return (
                <div key={i} className="history-item">
                  <div className="history-info">
                    <div className="history-result">{h.right}/{h.total} correct</div>
                    <div className="history-date">{dateStr} at {timeStr}</div>
                  </div>
